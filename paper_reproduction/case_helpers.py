...
This module contains common utilities used across fig05_case1_single_rule.ipynb,
fig06_case2_multi_rule.ipynb, fig07_case3_multi_group.ipynb, and fig08_behavioral_effects.ipynb.
"""
...
# PAPER COLOR SCHEME
# Two-palette rule used across all paper figures:
#   - categorical series -> Okabe-Ito qualitative palette (colorblind-safe)
#   - sequential/ordered quantities (density hexbins, rule counts,
#     elasticities) -> viridis (perceptually uniform, grayscale-monotone)
OKABE_ITO = {
    "current": "#0072B2",  # blue: current system / status quo
    "reform": "#D55E00",  # vermillion: TaxSolver reform (dashed)
    "data": "#56B4E9",  # sky blue: taxpayer scatter / constraint bars
    "highlight": "#E69F00",  # orange: named-taxpayer highlights (Jude/Laila)
    "group2": "#009E73",  # bluish green: second categorical series
    "neutral": "grey",
}
...
def viridis_colors(n: int, lo: float = 0.0, hi: float = 0.85):
    """Sample n ordered colors from viridis (for ordered categorical series)."""
    import numpy as np
    import matplotlib.pyplot as plt

    return plt.cm.viridis(np.linspace(lo, hi, n))


def rates_to_intervals(r_and_r: pd.DataFrame, rule_family: str = None) -> pd.DataFrame:
    """Convert a solver ``rules_and_rates_table()`` into the plotting interval
    format (``bracket_start``, ``bracket_end``, ``rate``).

    Bracket bounds are parsed from the trailing ``_<start>_<end>`` of each
    FlatTaxRule's ``var_name`` (e.g. ``income_before_tax_k_everybody_0_25000``).
    ``rule_family`` filters on the prefix of ``rule_name`` so group-specific
    bracket families can be extracted separately. This lets every figure be
    built directly from the notebook's own solve, with no pre-saved tables.
    """
    df = r_and_r[r_and_r["rule_type"] == "FlatTaxRule"].copy()
    if rule_family is not None:
        df = df[df["rule_name"].str.startswith(rule_family)]
    bounds = df["var_name"].str.extract(r"_(\d+)_(\d+)$").astype(float)
    df["bracket_start"] = bounds[0].values
    df["bracket_end"] = bounds[1].values
    df = df.dropna(subset=["bracket_start"]).sort_values("bracket_start")
    return df.reset_index(drop=True)


def benefit_value(r_and_r: pd.DataFrame, rule_name: str) -> float:
    """Solved value (rate) of a named benefit rule."""
    return float(r_and_r.loc[r_and_r["rule_name"] == rule_name, "rate"].iloc[0])


def pad_intervals(
    df: pd.DataFrame, lo: float = 0.0, hi: float = 1_000_000.0
) -> pd.DataFrame:
    """Pad an interval table with zero-rate segments so it spans [lo, hi].

    Used for group-specific bracket families (e.g. a healthcare taper defined
    only on [30k, 40k]) so their tax lines can be drawn over the full range.
    """
    rows = []
    if len(df) == 0:
        return pd.DataFrame([{"bracket_start": lo, "bracket_end": hi, "rate": 0.0}])
    if float(df["bracket_start"].iloc[0]) > lo:
        rows.append(
            {
                "bracket_start": lo,
                "bracket_end": float(df["bracket_start"].iloc[0]),
                "rate": 0.0,
            }
        )
    rows.extend(df[["bracket_start", "bracket_end", "rate"]].to_dict("records"))
    if float(df["bracket_end"].iloc[-1]) < hi:
        rows.append(
            {
                "bracket_start": float(df["bracket_end"].iloc[-1]),
                "bracket_end": hi,
                "rate": 0.0,
            }
        )
